...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{BENCHFARM_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{BENCHFARM_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
benchfarm_server.pl - Catalyst Testserver

=head1 SYNOPSIS

benchfarm_server.pl [options]
